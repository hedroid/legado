...
object LauncherIconHelp {
    ...
    fun normalizeIcon(icon: String?): String? {
        return when (icon) {
            "launcher5" -> "launcher2"
            "launcherw", "launcher6" -> "ic_launcher"
            else -> icon
        }
    }

    fun changeIcon(icon: String?) {
        val normalizedIcon = normalizeIcon(icon)
        if (normalizedIcon.isNullOrEmpty()) return
        var hasEnabled = false
        componentNames.forEach {
            if (normalizedIcon.equals(it.className.substringAfterLast("."), true)) {
                hasEnabled = true
                //启用
